...
async fn routes_to_model_and_returns_reply() {
    ...
    let mut agent = AgentLoop::open(&config, workspace.path(), "test_session", None, None).unwrap();
...
async fn command_routing_help_does_not_call_model() {
    ...
    let mut agent = AgentLoop::open(&config, workspace.path(), "test_cmd", None, None).unwrap();
...
async fn multi_turn_session_keeps_context_on_same_agent() {
    ...
    let mut agent = AgentLoop::open(&config, workspace.path(), "test_multi", None, None).unwrap();
...
async fn api_error_is_propagated() {
    ...
    let config = openai_config(&server.url());
    let workspace = TempDir::new().unwrap();
    let mut agent = AgentLoop::open(&config, workspace.path(), "test_error", None, None).unwrap();
